...
# Setup with species information
func _on_interaction_area_input(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		# Only allow selection of named animals
		if is_named:
			# Always emit selected signal when clicked
			...
			# If not named, request naming
			var animal_manager = get_node_or_null("/root/AnimalManager")
			if animal_manager:
				animal_manager.emit_signal("animal_naming_requested", self)
